...
protected:
    ...
    void kde_output_configuration_v2_set_allow_color_power_saving(Resource *resource, wl_resource *outputdevice, uint32_t allow) override;
};
...
void OutputConfigurationV2Interface::kde_output_configuration_v2_set_allow_color_power_saving(Resource *resource, wl_resource *outputdevice, uint32_t allow)
{
    if (invalid) {
        return;
    }
    if (OutputDeviceV2Interface *output = OutputDeviceV2Interface::get(outputdevice)) {
        config.changeSet(output->handle())->allowColorPowerSaving = allow;
    }
}
