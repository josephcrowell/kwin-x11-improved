...
bool ScreenShotEffect::takeScreenShot(const RenderTarget &renderTarget, const RenderViewport &viewport, ScreenShotAreaData *screenshot)
{
    if (!effects->waylandDisplay()) {
        ...
        QImage snapshot = blitScreenshot(renderTarget, viewport, screenshot->area, 1.0, ColorDescription::sRGB);
        if (screenshot->flags & ScreenShotIncludeCursor) {
...
QImage ScreenShotEffect::blitScreenshot(const RenderTarget &renderTarget, const RenderViewport &viewport, const QRect &geometry, qreal devicePixelRatio, const ColorDescription &colorspace) const
{
    ...
    if (OpenGlContext *context = effects->openglContext()) {
        ...
        if (renderTarget.texture()) {
            ...
            binder.shader()->setColorspaceUniforms(renderTarget.colorDescription(), colorspace, RenderingIntent::Perceptual);
