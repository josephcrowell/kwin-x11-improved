...
RegionF ContrastEffect::contrastRegion(const EffectWindow *w) const
{
    ...
    if (const auto it = m_windowData.find(w); it != m_windowData.end()) {
        const RegionF &appRegion = it->second.contrastRegion;
...
void ContrastEffect::uploadRegion(std::span<QVector2D> map, const RegionF &region, qreal scale)
{
    size_t index = 0;
...
{
    const int vertexCount = region.rects().size() * 6;
    if (!vertexCount) {
        return false;
    }
...
void ContrastEffect::drawWindow(const RenderTarget &renderTarget, const RenderViewport &viewport, EffectWindow *w, int mask, const QRegion &region, WindowPaintData &data)
{
    if (shouldContrast(w, mask, data)) {
        ...
        RegionF shape = contrastRegion(w).translated(w->pos());
        ...
        if (scaled) {
            ...
            for (QRectF r : shape) {
                ...
                const QPointF bottomRight(topLeft.x() + r.width() * data.xScale(), topLeft.y() + r.height() * data.yScale());
                scaledShape += QRectF(QPointF(topLeft.x(), topLeft.y()), bottomRight);
            }
            shape = scaledShape;

            // Only translated, not scaled
        } else if (translated) {
            shape.translate(QPointF(data.xTranslation(), data.yTranslation()));
        }

        const RegionF effectiveShape = shape & region & screen;
...
void ContrastEffect::doContrast(const RenderTarget &renderTarget, const RenderViewport &viewport, EffectWindow *w, const RegionF &shape, const float opacity, const QMatrix4x4 &screenProjection)
{
    ...
    vbo->draw(GL_TRIANGLES, 0, shape.rects().count() * 6);
