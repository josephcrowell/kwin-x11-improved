...
void main()
{
    ...
    tex = nitsToEncoding(tex, gamma22_EOTF, 0.0, destinationReferenceLuminance, destinationReferenceLuminance);
    tex.rgb /= max(0.001, tex.a);
    tex.rgb = vec3(1.0) - tex.rgb;
    tex *= modulation;
    tex.rgb *= tex.a;
    tex = encodingToNits(tex, gamma22_EOTF, 0.0, destinationReferenceLuminance, destinationReferenceLuminance);
